...
export class AttendanceModifyPage {
  ...
  launch(): void {
    this.subscriptions = [
      this.workFlow.getSpecificWorkFlowList(
        Observable.of({ process_id: ProcessIdOptions.attendanceModify, ...this.workFlow.getPendingOption() }),
  ...
  goToNextPage(target: MissionListItem): void {
    this.navCtrl.push(attendanceModifyDetailPage, { id: target.id }).then(() => { });
  }
