	...
	switch (action.type) {
		...
		case actions.CERTIFICATE_LIST_SUCCESS:
			return {
				...state,
				certificateListResponse: {
					work_certificates: action.payload.work_certificates.map(item => addPrefixForImage(item)),
				},
			};
		...
		case actions.RESET_WORKER_CERTIFICATE_ERROR_RESPONSE:
			return {
				...state,
				certificateListResponse: !!state.certificateListResponse.errorMessage
					? null
					: state.certificateListResponse,
				certificateAddResponse: !!state.certificateAddResponse.errorMessage
					? null
					: state.certificateAddResponse,
				certificateUpdateResponse: !!state.certificateUpdateResponse.errorMessage
					? null
					: state.certificateUpdateResponse,
				certificateDeleteResponse: !!state.certificateDeleteResponse.errorMessage
					? null
