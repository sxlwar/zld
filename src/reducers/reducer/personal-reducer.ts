...
export function reducer(state = initialState, action: actions.Actions): State {
	switch (action.type) {
		...
		case actions.BASIC_INFORMATION_SUCCESS:
			return { ...state, basicResponse: action.payload };
		...
		case actions.UPDATE_WORKER_DETAIL_SUCCESS:
			return {
				...state,
				workerDetailUpdateResponse: action.payload,
				workerDetailResponse: {
					workers: [ updateAddress(state.workerDetailResponse.workers[0], state.workerDetailOptions) ],
				},
			};
		...
		case actions.UPDATE_WORK_TYPES_AT_LOCAL: {
			return {
				...state,
				workerDetailResponse: {
					workers: [ { ...state.workerDetailResponse.workers[0], workType__id: action.payload } ],
				},
			};
		}
		...
		case actions.HOME_INFO_UPDATE_SUCCESS:
			return {
				...state,
				homeInfoUpdateResponse: action.payload,
				homeInfoResponse: {
					home_info: [ updateHomeInfo(state.homeInfoResponse.home_info[0], state.homeInfoOptions) ],
				},
			};
		...
		case actions.ADD_EDUCATION_SUCCESS:
			return {
				...state,
				educationResponse: {
					education: [ ...state.educationResponse.education, omit(state.educationAddOptions, [ 'sid' ]) ],
				},
			};
		...
		case actions.DELETE_EDUCATION_SUCCESS:
			return {
				...state,
				educationDeleteResponse: action.payload,
				educationResponse: {
					education: state.educationResponse.education.filter(
						item => item.id !== state.educationDeleteOptions.education_id
					),
				},
			};
		...
		case actions.UPDATE_EDUCATION_SUCCESS:
			return {
				...state,
				educationUpdateResponse: action.payload,
				educationResponse: {
					education: state.educationResponse.education.map(
						item => item.id === state.educationUpdateOptions.id
								? { ...item, ...omit(state.educationUpdateOptions, [ 'sid' ]) }
								: item
					),
				},
			};
		...
		case actions.WORK_EXPERIENCE_ADD_SUCCESS:
			return {
				...state,
				workExperienceAddResponse: action.payload,
				workExperienceResponse: {
					exp_add: [
						...state.workExperienceResponse.exp_add,
						omit(state.workExperienceAddOptions, [ 'sid' ]),
					],
				},
			};
		...
		case actions.WORK_EXPERIENCE_DELETE_SUCCESS:
			return {
				...state,
				workExperienceDeleteResponse: action.payload,
				workExperienceResponse: {
					exp_add: state.workExperienceResponse.exp_add.filter(
						item => item.id !== state.workExperienceDeleteOptions.workexper_id
					),
				},
			};
		...
		case actions.WORK_EXPERIENCE_UPDATE_SUCCESS:
			return {
				...state,
				workExperienceUpdateResponse: action.payload,
				workExperienceResponse: {
					exp_add: state.workExperienceResponse.exp_add.map(
						item => item.id === state.workExperienceUpdateOptions.id
								? { ...item, ...omit(state.workExperienceUpdateOptions, [ 'sid' ]) }
								: item
					),
				},
			};
...
export function updateAddress(source: WorkerDetail, option: WorkerDetailUpdateOptions): WorkerDetail {
	...
	if (option.province)
		Object.assign(result, {
			curraddr__province: option.province,
			curraddr__city: option.city,
			curraddr__dist: option.dist,
		});
...
export function updateHomeInfo(source: Home, option: HomeInfoUpdateOptions): Home {
	const result = omit(option, [ 'sid', 'user_id' ]);
